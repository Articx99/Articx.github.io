...
---
**Hackthebox - Search - Writeup**
Lets begin with a tcp port scan using nmap to discover open ports in the machine.
...
```
now lets scan more deeply these ports we found to use some basic scripts and see the service and the version they running.
Before that, i have these utility that extract all the importan information and copy the ports in the clipboard. 
So you dont need to write all of them.
If you want this tool copy this code and paste it in your .bashrc or your .zshrc 
```console
...
```
Theres a lot of ports so lets start with basic recognizement.
```console
...
```
I cant connect to smb with null session but i saw the domain name "search.htb" so we add it in /etc/hosts, check rdp with null session but access denied, so we go check the web.
Analycing the web i noticed a weird image, so i check it up in a new tab with more zoom.
![image found](/assets/images/weird.png)
Found user "Hope sharp" and password "IsolationIsKey?", but i dont know the user policy they using so i brute force and i create a diccionary with posibles combinations.
![image user](/assets/images/hopesharpuser.png)
Then use crackmapexec to check which is the good one.
```console
...
```
hope.sharp is the correct one so we use rpd and smb to get more information now we have credentials.
```console
...
....
```
A lot of stuff but we only need the usernames so we filter what is inside of [] and save it in users.txt
```console
...
```
Now with the users file lets test if some user in the list is vulnerable to ASREPRoast attack
```console
❯ GetNPUsers.py -usersfile users.txt search.htb/
```
But they are not. So i tried kerberoasting attack and gaining a TGS with hope.sharp authentication i have.
```console
...
```
But we run into this error because we need to syncronice the clock with the server so i use ntpdate.
```console
...
Session completed
```
Now we have these new credential lets 
```console
...
If you open now the document.xlsx you can watch the credentials.
...
```
Found that Sierra.Fry has a correct password so we try smbmap to see if she had Something in his folders
```console
...
```
Found backups with some pfx files, download it and test if i can import it in the browser, but credentials are needed. So with pfx2john get hash and crack it.
Then import the certs in the browser.
![image certs](/assets/images/certs.png)
Now that we have certs we need to find a page to use them, so we do fuzzing.
```console
...
We try the sierra credentials and we are in(in domain you have to put RESEARCH, we found that early in crackmapexec).
Now its time to use BloodHound, so first we need the js files so lets use BloodHound-python with the credentials we have.
```console
...
.....
```
Import the files we get to the bloodhound and mark the users we pwnd as owned.
then we click on short path to get domain admin and we get Something like this:
![image certs](/assets/images/bloodhound1.png)
Now you can see in the bloodhound that you can gain access to BIR-ADFS-GMSA via ReadGMSAPassword. So check this page that explain it perfectly.
...
Execute net user and change tristan.davies password and get domain admin.
```console
